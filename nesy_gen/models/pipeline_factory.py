...
def build_primekg_pipeline(
    ...
    *,
    ...
    if shuffled_relations:
        import numpy as np
        rng = np.random.default_rng(seed=42)
        kg.edges["relation"] = rng.permutation(kg.edges["relation"].values)
        kg.edges["display_relation"] = kg.edges["relation"]
        from nesy_gen.kg.primekg import PrimeKGGraph as _PKG
        kg = _PKG.from_dataframe(kg.edges)
