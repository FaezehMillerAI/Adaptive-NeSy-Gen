"""Ablation suite for Adaptive NeSy-Gen with MedGemma training-free backend.

Ablations (paper Section 13):
 1. retrieval_only        — top-1 retrieved report as prediction
 2. no_verify             — MedGemma draft with no verification
 3. no_graph              — RAG + gate but no PrimeKG/LTN
 4. report_level          — report-level (not claim-level) verification
 5. full_adaptive         — proposed method (default)
 6. always_on             — verification without fast path (τ_fast=0)
 7. no_ltn                — PrimeKG connectivity only, LTN disabled
 8. no_gate               — PrimeKG/LTN without Consistency Gate
 9. shuffled_kg           — shuffled PrimeKG relations (control)
"""
